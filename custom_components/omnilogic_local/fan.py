    ...
    for system_id, pump in filters_and_pumps.items():
        ...
        match pump.msp_config.type:
            case PumpType.VARIABLE_SPEED:
                entities.append(OmniLogicPumpFanEntity(coordinator=coordinator, context=system_id))
            case FilterType.VARIABLE_SPEED:
                entities.append(OmniLogicFilterFanEntity(coordinator=coordinator, context=system_id))
...
class OmniLogicVSPFanEntity(OmniLogicEntity[T], FanEntity):
    ...
    @property
    def name(self) -> Any:
        return f"{self.bow_id if self.bow_id is not None else 'Backyard'} {self.data.msp_config.name} Fan"
    ...
    @property
    def max_speed_pct(self) -> int:
        return self.data.msp_config.max_percent

    @property
    def min_speed_pct(self) -> int:
...
class OmniLogicPumpFanEntity(OmniLogicVSPFanEntity[EntityIndexPump]):
    ...
    async def async_set_percentage(self, percentage: float = None) -> None:
        """Update the current value."""
        if percentage is None:
            percentage = self.native_value

        if self.native_unit_of_measurement == "RPM":
            new_speed_pct = round(percentage / self.native_max_value * 100)
        else:
            new_speed_pct = int(percentage)
    ...
    async def async_turn_on(self, preset_mode, **kwargs) -> None:
        """Turn the entity on."""
